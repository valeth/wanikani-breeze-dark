...
CSS_OUT        = 'compiled/wanikani_breeze_dark.user.css'

def css_configured(configfile)
  config_name = File.basename(configfile, '.yml')
  "compiled/wanikani_breeze_dark-#{config_name}.user.css"
end
...
task :replace, [:configfile] => :build do |_t, args|
  ...
  open(CSS_OUT) do |cssfile|
    tmp = cssfile.read
    open(css_configured(configfile), 'w') do |outfile|
...
desc 'Copy the generated out.user.css to the clipboard (requires xsel)'
task :copy, %i[replace configfile] => :build do |_t, args|
  if args[:replace]
    configfile = args[:configfile]
    outfile = css_configured(configfile)
    Rake::Task[:replace].invoke(configfile)
    sh "xclip -selection clipboard #{outfile}"
    notify("Copied #{outfile} to clipboard.")
  else
    sh "xclip -selection clipboard #{CSS_OUT}"
    notify("Copied #{CSS_OUT} to clipboard.")
  end
end
